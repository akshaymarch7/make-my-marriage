...
export function WorkspaceFrame({ children, user, role }: { children: ReactNode; user: { name: string }; role?: string }) {
  ...
  const section = pathname.startsWith("/tasks") ? "Tasks" : pathname.startsWith("/events") ? "Events" : pathname === "/wedding/edit" ? "Wedding details" : pathname === "/settings/members" ? "Wedding members" : "Dashboard";
