...
it("keeps navigation on existing routes and restricts member management to Admins", async () => {
  ...
  expect([...sidebar.querySelectorAll("a")].map(a => a.getAttribute("href"))).toEqual(["/", "/dashboard", "/events", "/tasks", "/wedding/edit", "/settings/members"]);
...
it("opens every V1 placeholder without making a network request", async () => {
  ...
  expect(buttons).toHaveLength(Object.keys(plannedFeatures).length - 2);
  for (const [index, feature] of Object.entries(plannedFeatures).filter(([key]) => key !== "events" && key !== "tasks").map(([, value]) => value).entries()) {
