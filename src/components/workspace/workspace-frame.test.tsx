...
it("keeps navigation on existing routes and restricts member management to Admins", async () => {
  ...
  expect([...sidebar.querySelectorAll("a")].map(a => a.getAttribute("href"))).toEqual(["/", "/dashboard", "/events", "/tasks", "/guests", "/dashboard#rsvp-summary", "/gallery", "/wedding/edit", "/settings/members"]);
...
it("opens every V1 placeholder without making a network request", async () => {
  ...
  expect(buttons).toHaveLength(Object.keys(plannedFeatures).length - 5);
  for (const [index, feature] of Object.entries(plannedFeatures).filter(([key]) => key !== "events" && key !== "tasks" && key !== "guests" && key !== "invitations" && key !== "gallery").map(([, value]) => value).entries()) {
